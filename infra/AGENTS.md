...
- Deploy disk preflight requires both acceptable utilization and at least 6 GiB free whenever any selected image needs a build. It may skip the build-capacity gate only when every selected ref is the verified target SHA and already exists locally; that rollout is reuse-only and refuses a fallback build if an image disappears. `MAXIM_DEPLOY_DISK_MIN_FREE_BYTES` may raise but never lower the hard floor, and the percentage emergency override does not bypass it. Clean shared API builds have exceeded 15 GiB of temporary usage, so prefer a preloaded reviewed image or more disk capacity when the host is below that observed demand.
