...
- Local deploy resolves the selected branch to an exact SHA, requires a successful aggregate `Required` CI check for that SHA, and sends it to the VPS for a post-sync `HEAD` equality check. Emergency bypass requires both `MAXIM_DEPLOY_EMERGENCY_BYPASS=1` and `MAXIM_DEPLOY_EMERGENCY_REASON`.
