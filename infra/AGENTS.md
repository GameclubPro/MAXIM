...
- Component manifests live under `/var/lib/maxim-deploy`, retain at least five releases, and track each active component's source SHA, image ref, and image ID. The atomic `current.json` update happens only after image-ID verification and strict component smokes; an incomplete deploy after runtime mutation invalidates stale current inventory, matching both rollback paths.
