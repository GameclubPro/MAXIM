...
function collectUrlMatches(value: string): UrlMatch[] {
  ...
  const fullSchemeMatches = hasSchemeCandidate
    ? collectMatches(prepared.text, createSchemeUrlRegex(), prepared.originalIndexBySourceIndex)
    : [];
...
export function getUrlTextRanges(value: string): Array<{ start: number; end: number }> {
  return collectUrlMatches(value).map(({ start, end }) => ({ start, end }));
}
