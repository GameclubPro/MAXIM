...
export const surfaceTargetSchema = z.enum(['miniapp', 'private_bot']);
export type SurfaceTarget = z.infer<typeof surfaceTargetSchema>;

export const surfaceSourceSchema = z.enum([
  'miniapp',
  'private_bot',
  'chat_inline',
  'channel_inline',
...
export type SurfaceSource = z.infer<typeof surfaceSourceSchema>;

export const surfaceEntryRequestSchema = z.object({
  intent: surfaceIntentSchema,
  entityType: managedEntityTypeSchema,
  entityId: z.string().trim().min(1),
  section: z.string().trim().min(1).max(64).nullable().optional().default(null),
  dialogType: z.enum(['comments', 'suggest']).nullable().optional().default(null),
  resourceId: z.string().trim().min(1).nullable().optional().default(null),
  sourceSurface: surfaceSourceSchema.default('miniapp'),
});
export type SurfaceEntryRequest = z.infer<typeof surfaceEntryRequestSchema>;

export const surfaceEntryResponseSchema = z.object({
  targetSurface: surfaceTargetSchema,
  botUrl: z.string().url().nullable().optional().default(null),
  miniappUrl: z.string().url().nullable().optional().default(null),
  startParam: z.string().trim().min(1).nullable().optional().default(null),
  resumeToken: z.string().trim().min(1).nullable().optional().default(null),
  fallbackUrl: z.string().url(),
});
export type SurfaceEntryResponse = z.infer<typeof surfaceEntryResponseSchema>;

export const workbenchChecklistKeySchema = z.enum([
  'entity_visible',
  'bot_admin_granted',
  'api_ready',
  'first_setting_saved',
...
export type WorkbenchChecklistKey = z.infer<typeof workbenchChecklistKeySchema>;

export const workbenchChecklistItemSchema = z.object({
  key: workbenchChecklistKeySchema,
  label: z.string(),
  description: z.string().nullable().default(null),
  done: z.boolean(),
});
export type WorkbenchChecklistItem = z.infer<typeof workbenchChecklistItemSchema>;

export const workbenchActivationSchema = z.object({
  completed: z.boolean(),
  completedSteps: z.number().int().min(0),
  totalSteps: z.number().int().min(1),
  nextStepLabel: z.string().nullable().default(null),
  items: z.array(workbenchChecklistItemSchema),
});
export type WorkbenchActivation = z.infer<typeof workbenchActivationSchema>;

export const workbenchAttentionSchema = z.object({
  moderationEvents24h: z.number().int().min(0),
  activeBroadcasts: z.number().int().min(0),
  pendingBroadcastDeliveries: z.number().int().min(0),
  activeGiveaways: z.number().int().min(0),
  hasPollDraft: z.boolean(),
});
export type WorkbenchAttention = z.infer<typeof workbenchAttentionSchema>;

export const workbenchQuickActionKeySchema = z.enum([
  'open_miniapp',
  'open_settings',
  'open_events',
  'manual_action',
...
export type WorkbenchQuickActionKey = z.infer<typeof workbenchQuickActionKeySchema>;

export const workbenchQuickActionSchema = z.object({
  key: workbenchQuickActionKeySchema,
  label: z.string(),
  description: z.string().nullable().default(null),
  intent: surfaceIntentSchema,
  targetSurface: surfaceTargetSchema,
  enabled: z.boolean().default(true),
  section: z.string().trim().min(1).nullable().optional().default(null),
});
export type WorkbenchQuickAction = z.infer<typeof workbenchQuickActionSchema>;

export const workbenchDraftKindSchema = z.enum(['broadcast', 'poll', 'giveaway']);
export type WorkbenchDraftKind = z.infer<typeof workbenchDraftKindSchema>;

export const workbenchDraftSchema = z.object({
  kind: workbenchDraftKindSchema,
...
export const workbenchSummarySchema = z.object({
  header: managedEntityHeaderSchema,
  activation: workbenchActivationSchema,
  attention: workbenchAttentionSchema,
  quickActions: z.array(workbenchQuickActionSchema),
  activeDrafts: z.array(workbenchDraftSchema),
  defaultSection: z.string().trim().min(1).nullable().default(null),
  legacyInlineFallbackAvailable: z.boolean().default(true),
});
...
export const broadcastHandoffResponseSchema = z.object({
  botUrl: z.string().url(),
  targetSurface: surfaceTargetSchema.optional().default('private_bot'),
  miniappUrl: z.string().url().nullable().optional().default(null),
  startParam: z.string().trim().min(1).nullable().optional().default(null),
  resumeToken: z.string().trim().min(1).nullable().optional().default(null),
  fallbackUrl: z.string().url().nullable().optional().default(null),
});
