...
- Moderation enforcement and retry-critical moderation cleanup deletes must use durable `ModerationDeleteIntentService` intents. Keep direct deletes only inside intent execution or explicit shadow/off compatibility paths; `MODERATION_DELETE_CROSS_BOT_CANARY_CHAT_IDS` is an execution-time kill switch that existing intents must honor.
- A MAX message delete succeeds only on the documented `{ success: true }` response or a message-specific absence confirmation. Never treat an arbitrary HTTP `404` as proof that the target message is gone.
...
- `/publications` `NOW` is user-triggered even when the action-role poller performs recovery: materialize it ahead of background work and dispatch it through the immediate/interactive lane. Keep DB-only publication rollups outside background governor pauses, and keep ambiguous sends manual-review-only.
- Publication retries must preserve the delivery's recorded content revision by default. Retrying with latest content requires explicit optimistic publication/content revision guards; never rewrite SENT or AMBIGUOUS delivery attribution.
- Publication video input remains capped at 24 MB because it crosses base64 JSON, Postgres `bytea`, and in-memory buffers. Resumable MAX upload support is outbound-only and must not be used to raise that product limit without a separate ingestion/storage redesign.
