...
- `major-maksimov.ru`, `www.major-maksimov.ru`, and `app.major-maksimov.ru` currently enter through the Yandex edge IP `84.201.186.244`; `app.major-maksimov.ru` should redirect to the canonical `major-maksimov.ru`. Keep `/app/` as the production mini app and keep the root page clear of internal infrastructure details.
- `app2.major-maksimov.ru/app/` is the operator-reachable production candidate for the mini app shell. It is served by Yandex CDN in front of Object Storage bucket `flex-craft-canary-20260608`, prefix `app/`; verify it with a cache-busting query and expect `x-major-app-cdn-canary: yandex-cdn-prod-app2`.
- For the Object Storage mini app path, upload `apps/miniapp/dist` under `s3://flex-craft-canary-20260608/app/`: hashed `assets/*` should use `Cache-Control: public, max-age=31536000, immutable`, while `app/`, `app/index.html`, `favicon.png`, `apple-touch-icon.png`, and `site.webmanifest` should use `no-store, no-cache, must-revalidate, max-age=0`.
