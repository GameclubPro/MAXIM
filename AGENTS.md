...
- `ChatSettings.antiSpamEnabled` is the fast per-chat sender flood guard for plain non-media messages only. The hard threshold is `MESSAGE_RATE_LIMIT` on the 6th non-media message within 6 seconds; exclude media attachments, media batches, and forward-only linked messages from this counter, do not expose threshold controls, and hard-ban only matching non-media burst senders instead of routing them through configurable message-limit escalation.
