...
- CDN/app2 mini app delivery is paused as the default path. The current primary user-facing host is `https://maxim.play-team.ru/`; for routine mini app work, deploy and verify the direct VPS/origin app path at `https://maxim.play-team.ru/app/`; publish or smoke `app2.major-maksimov.ru` only when the user explicitly asks for CDN/restricted-LTE work.
...
- Use `https://maxim.play-team.ru/` as the primary user-facing host and `https://maxim.play-team.ru/app/` as the primary direct mini app URL while CDN/app2 delivery is paused. Do not publish Object Storage or run `app2.major-maksimov.ru` CDN smokes for routine mini app changes unless the user explicitly resumes CDN/restricted-LTE work.
...
- For the Object Storage mini app path, upload `apps/miniapp/dist` under `s3://flex-craft-canary-20260608/app/`: hashed `assets/*` should use `Cache-Control: public, max-age=31536000, immutable`, while `app/`, `app/index.html`, `favicon.png`, `apple-touch-icon.png`, and `site.webmanifest` should use `no-store, no-cache, must-revalidate, max-age=0`. Set object content types explicitly: JavaScript assets must be `application/javascript; charset=utf-8`, CSS assets `text/css; charset=utf-8`, and HTML shell keys `text/html; charset=utf-8`; module scripts served as `text/plain` render a blank mini app. If old headers are stuck in CDN `HIT`, purge the CDN resource or deploy a versioned asset prefix and update the HTML.
- Restricted-LTE smoke for the Major mini app: `curl -I 'https://app2.major-maksimov.ru/app/?v=<stamp>'` should return `200`, `text/html`, `no-store`, and `x-major-app-cdn-canary: yandex-cdn-prod-app2`; referenced JS/CSS assets should return correct MIME types; `https://api-cdn.flex-craft.ru/api/health/live` and core `/api/v1` reads should answer through CDN; preflight from `Origin: https://app2.major-maksimov.ru` should allow `authorization, content-type`; browser/Playwright smoke should require non-empty `#root`.
- Common restricted-LTE failure signatures: if `major-maksimov.ru/app/` fails only on MTS/Beeline but works on Megafon/Wi-Fi, treat it as front-door reachability first, not nginx/TLS/app; if `app2` opens blank, check Object Storage/CDN asset MIME; if reads work but settings/sends fail, check CDN 405 and the mutation tunnel allowlist before changing business logic.
- If `yc storage s3 cp` or `yc storage s3api put-object` hangs from the local network, use `yc iam create-token` and direct HTTPS `PUT` to `https://storage.yandexcloud.net/flex-craft-canary-20260608/<key>` with `Authorization: Bearer <token>`. A local SOCKS tunnel through `maxim-vk-jump` (`ssh -fN -D 127.0.0.1:10880 maxim-vk-jump`) is a proven workaround for local Object Storage timeouts; do not store IAM tokens after use.
