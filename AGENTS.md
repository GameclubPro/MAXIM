...
- VK parsing imports at most one direct HTTPS `video.files.mp4_*` URL per VK post. Do not scrape VK `player`, accept `external`/HLS as a video file, or mix photos and video in one MAX publish payload; when a supported video is present, publish video and drop photos from that post's publishable media set.
