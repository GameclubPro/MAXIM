...
- Global spammer observations, scoring, suppression, and review decisions live in `GlobalSpammerIntelligenceService`; active `global_spammers` rows must have a future `expiresAt` because nullable legacy rows are treated as expired, and admin review endpoints should stay in `ManualModerationService` instead of growing legacy `AdminService`.
