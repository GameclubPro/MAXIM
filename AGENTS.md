...
- Night mode and manual group close should stay webhook-only delete gates: delete incoming non-admin user messages in `handleUpdate`, do not add background chat scans, `listMessages` polling, bot notices, or greeting sends while the chat is closed.
