...
- `major-maksimov.ru` keeps a separate public root landing at `infra/www/major-maksimov/index.html`, applied with `./infra/scripts/vps-apply-major-site.sh`; keep `/app/` as the production mini app and keep the root page clear of internal infrastructure details.
- `app.major-maksimov.ru` is the VK Cloud VM proxy for MAX mini app traffic; Yandex DNS points it to the VK VM public IP, nginx config and public root live under `infra/vk-proxy/`, and `./infra/scripts/vk-apply-major-proxy.sh [ubuntu@ip]` reapplies them. Create/rebuild the VK VM with OpenStack `--use-config-drive` so SSH keys and network config are injected on the direct `internet` network. Keep backend `APP_BASE_URL=https://app.major-maksimov.ru`, but leave `MAX_WEBHOOK_BASE_URL=https://maxim.play-team.ru` unless intentionally moving webhooks.
