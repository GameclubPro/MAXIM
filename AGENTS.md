...
- `app2.major-maksimov.ru/app/` is the operator-reachable production mini app shell for restricted LTE. It is served by Yandex CDN in front of Object Storage bucket `flex-craft-canary-20260608`, prefix `app/`; verify it with a cache-busting query and expect `x-major-app-cdn-canary: yandex-cdn-prod-app2`. Use this URL for MAX mini app settings when restricted-LTE reachability matters; direct Object Storage URLs are diagnostic/provider fallbacks, and `major-maksimov.ru/app/` remains the origin/direct path for already-reachable networks.
- The `app2` shell build must keep `VITE_PUBLIC_BASE_PATH=/app/`, `VITE_API_BASE=https://api-cdn.flex-craft.ru/api/v1`, and `VITE_API_FALLBACK_BASES=https://major-maksimov.ru/api/v1`. The mini app transport prefers `api-cdn.flex-craft.ru` and falls back to `major-maksimov.ru` when that direct origin is reachable.
