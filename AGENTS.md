...
- For admin contact links, prefer a direct HTTP(S) `profileUrl` when MAX provides one, then fall back to HTTP(S) `profileHandoffUrl`; most MAX users do not have a public username/profile URL. Do not convert fixed-label `Связь с админом` links to `max://user/<id>` because MAX can render arbitrary-label user mentions as plain text.
