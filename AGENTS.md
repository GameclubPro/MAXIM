...
- VK parsing automation settings are stored per managed entity in `vk_parsing_settings`. Scheduled/manual sync can autopublish newly imported posts, but the initial `source-added` backfill must not autopublish old fetched posts.
