...
- `major-maksimov.ru`, `www.major-maksimov.ru`, and `app.major-maksimov.ru` enter through the VK Cloud VM proxy; Yandex DNS points them to the VK VM public IP, while the VK nginx upstream still proxies `/app/` and `/api/` to the Yandex origin `84.201.186.244:443`. Public root assets live under `infra/www/major-maksimov/`, VK nginx config lives under `infra/vk-proxy/`, and `./infra/scripts/vk-apply-major-proxy.sh [ubuntu@ip]` reapplies them. Keep `/app/` as the production mini app and keep the root page clear of internal infrastructure details.
- Create/rebuild the VK VM with OpenStack `--use-config-drive` so SSH keys and network config are injected on the direct `internet` network. The VK proxy certificate should cover `major-maksimov.ru`, `www.major-maksimov.ru`, and `app.major-maksimov.ru`, and renewal should use the `/.well-known/acme-challenge/` webroot on the VK VM. Leave `MAX_WEBHOOK_BASE_URL=https://maxim.play-team.ru` unless intentionally moving webhooks.
