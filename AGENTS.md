...
- Stop-words settings are their own section and include both `messageLimitsBlockedWords` and `messageLimitsBlockedDomains`; blocked domains match exact hosts and subdomains independently of link moderation policy.
- Link allowlist `DOMAIN` rules (`domain:example.com`) match the exact host and its subdomains; `EXACT` rules stay URL-specific.
...
- Global spammer observations, scoring, suppression, graph signals, source reputation, policy diagnostics, and review decisions live in `GlobalSpammerIntelligenceService`; active `global_spammers` rows must have a future `expiresAt` because nullable legacy rows are treated as expired. Runtime sanctions should be gated by `evaluatePolicy`/active confirmed decisions, and admin review/diagnostic endpoints should stay in `ManualModerationService` instead of growing legacy `AdminService`.
...
- For sanitized commercial corpus exports, pass `--export-corpus-jsonl <path>` to the audit command and add `--export-all-corpus` when validating corpus gates so stable-clear negative candidates are included. Validate with `npm run moderation:validate-commercial-corpus --workspace @maxim/api -- --input <path>`.
