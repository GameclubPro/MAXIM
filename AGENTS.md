...
- Night mode and manual group close should stay webhook-only delete gates for incoming user messages: delete non-admin messages in `handleUpdate`, do not add background chat scans, `listMessages` polling, per-chat sleeps, user-message-triggered bot notices, or greeting sends while the chat is closed.
