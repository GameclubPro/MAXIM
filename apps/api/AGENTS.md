...
- `publikEnabled` is Publisher-owned and remains effectively enabled when no policy row exists. Policy reads/writes require an exact Publisher access edge, active catalog row, and binding; Major access is insufficient. A persisted `false` remains off, excludes publication target resolution and refresh, but stays visible in the Publisher list/get cabinet for explicit re-enablement.
