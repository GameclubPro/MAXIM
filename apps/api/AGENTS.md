...
- `api-media-analysis` owns OCR admission, source download, and policy; production native Sharp/Tesseract work runs only in the no-network/no-secret `ocr-native-sandbox` over bounded UDS IPC. Keep both single-concurrency and resource-capped, verify normal Tesseract process-group teardown, recycle the whole sandbox after forced native timeouts or abandoned in-flight native work, and keep Commercial OCR in `shadow` until the sandbox-backed identity passes the corpus and certification gates.
- Image stop-list OCR is a separate explicit per-chat, delete-only policy. Require two high-confidence word-span passes, keep OCR text out of jobs/DB/logs, persist its strict binding under the rollback-safe `COMMERCIAL_OCR_DELETE` sentinel, and re-check runtime, settings, active domain allowlist, author access/immunity, exact photo identity, native behavior, and the absolute deadline before every delete dispatch.
...
- `publikEnabled` is Publisher-owned and remains effectively enabled when no policy row exists. Policy reads/writes require an exact Publisher access edge, active catalog row, and binding; Major access is insufficient. A persisted `false` remains off and excludes publication target resolution, but stays visible in the Publisher cabinet for explicit re-enablement. Publisher access refresh, candidate verification, and their recovery must work while publishing is off, without writing the publication policy or sending posts.
...
- VK automatic imports create their pending schedule fingerprint under the same chat policy lock as source/settings mutations, and only for newly observed posts inside the current global/source enablement baseline. Schedule changes may rekey only unattempted intents; worker deferrals after any publish attempt must preserve the original intent key and MAX ledger identity.
- A MAX-confirmed VK result whose database commit is pending is a receipt-first, database-only recovery state. Finalize it before runtime, identity, health, source, age, or content-policy guards, and never overwrite its quarantine marker from another mutation path.
- Publication HTTP handlers, occurrence dispatch, and recurring materialization are Publisher-only. Major broadcast/autopost controllers and runners are unregistered; its settings screens return an empty legacy `managedBroadcasts` compatibility field without reading broadcasts. The shared action role may materialize Publisher work and roll up persisted receipts, but must not dispatch legacy envelopes. `ManagedBroadcastService` execution lanes run only in the Publisher role; never restore `LEGACY_ROUTED` sends.
- Retired private-bot broadcast callbacks, start payloads, and persisted input sessions return the Publik bot link without capturing media or submitting a send. Publishing handoff HTTP routes are retired; moderation commands, rules, polls, and giveaways remain independent.
...
- Legacy inline Publication video remains capped at 24 MB across base64 JSON, `bytea`, and in-memory buffers. New mini app video selection accepts up to 100 MB through direct browser-to-MAX upload; never raise JSON/body limits for it. `publisher-video-upload` runs only in `api-publisher`, confirms MAX readiness, and persists an actor-owned, exact-bot remote token with `bytes: null`. Public input must never mint its internal marker. Repeated publication reuses that token, not local video bytes.
...
- Required-subscription moderation checks every target with bounded concurrency and confirms stale missing cache through MAX before sanctions. Real moderation probes bypass the broader MAX member cache. Shadow membership mirrors reuse canonical preparation; only the winning canonical or recovery job repeats webhook invalidation after its execution routing/lock guard. A renewable message- or media-package-scoped lease serializes mirrored enforcement. Every ordinary confirmed violation must complete a durable user-facing notice handoff before creating its delete intent; media packages may share one durable delivered notice only after each covered message persists exact coverage, and only the package leader advances escalation. Do not add a broad user cooldown or optional-pressure skip. Save only verifiable targets; disable the feature if none remain and fail open on terminal target errors.
