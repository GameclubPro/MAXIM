...
- Mini app VK parsing routes authorize only the Publisher profile and exact Publisher entity access. Major chat settings writes, including bulk apply, must preserve Publisher-owned chat-comment fields.
...
- VK publish intent routing is durable: legacy queued rows stay on `vk-parsing-publish`/`api-action`, while newly armed `PUBLIK_V1` rows use the Publisher bot for both `requiredBotId` and `dialogBotId` and run only on `vk-parsing-publisher`/`api-publisher`. Historical immutable PUBLIK envelopes with a distinct validated dialog bot remain executable. Publisher dialog links use the domain-separated file key mounted only in `api-admin`, `api-action`, and `api-publisher`; never widen bot-token or init-data-key mounts to prepare links.
