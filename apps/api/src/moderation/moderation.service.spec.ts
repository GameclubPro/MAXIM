...
describe('ModerationService', () => {
  ...
  it('sends instruction for personal bot_started update and skips moderation flow', async () => {
    ...
    expect(maxClient.sendMessage).toHaveBeenCalledWith(
      '152517912',
      [
        'Дежурная часть «Майор Максимов» на связи. Чат принят под наблюдение.',
        '',
        'Что умею:',
        '- Ссылки: режу подозрительные URL, поддерживаю режим allowlist по доменам.',
        '- Текст: фильтрую мат и коммерческий спам.',
        '- Повторы: первый дубль удаляю, дальше по протоколу - предупреждение, кик, бан.',
        '- Ограничения: лимит длины, кулдаун фото, запрет видео/файлов/голосовых.',
        '- Режимы: ночной режим, приветствия новичков, автоудаление сообщений бота.',
        '- Контроль состава: выгон ботов из группы и глобальный черный список пользователей.',
        '- Рассылка: сообщения в чаты (текст/кнопка/фото), сразу или по таймеру.',
        '',
        'Панель управления в mini app: откройте бота в MAX и нажмите «Открыть» (open_app).',
        'Там настраиваются правила, тексты уведомлений и ступени наказаний.',
        '',
        'Схема простая: сначала беседа, потом протокол. Черный юмор выключить нельзя.',
      ].join('\n'),
    );
    expect(ruleEngine.detect).not.toHaveBeenCalled();
