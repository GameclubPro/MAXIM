...
describe('bot speech styles', () => {
  ...
  it('renders robot, friendly and ironic templates with style presets', () => {
    ...
    expect((service as any).buildGreetingMessage(userLabel, '', 'IRONIC')).toBe(
      '**Алексей**, привет 😏 На связи Майор Максимов. Осваивайтесь спокойно, а драму оставим сериалам.',
    );
    expect(
      (service as any).buildNightModeOpenedNotice(23 * 60, 8 * 60, 'Europe/Moscow', '', 'IRONIC'),
    ).toBe('☀️ Антракт окончен. Группа снова открыта. Можно снова писать, только без лишних соло.');

    expect((service as any).buildLinkExplanation(userLabel, true, '', 'IRONIC')).toBe(
      '**Алексей**, ссылку убрал 🔗 Внешний интернет тут сегодня без гастролей.',
    );

    expect((service as any).buildDuplicateHitExplanation(userLabel, true, '', 'IRONIC')).toBe(
      '**Алексей**, мысль уже выходила на сцену 🎭 ♻️ Повтор убрал. На бис тут не вызывают.',
    );
    ...
    expect(
      ...
      '**Алексей**, мысль уже выходила на сцену 🎭 ⚠️ Это уже предупреждение. На бис тут выходят не до бесконечности.',
    );
  });
  ...
  it('keeps system notices on the selected base style when one editable field is overridden', () => {
    ...
    expect(
      ...
      '**Алексей**, это уже предупреждение ⚠️ Причина: слишком длинное сообщение. Лимиты считают честно, даже когда это обидно.',
    );

    expect((service as any).buildLinkMuteExplanation(userLabel, 'IRONIC')).toBe(
      '**Алексей**, со ссылками пошли на бис 🔒 Поэтому теперь мут.',
    );
  });
