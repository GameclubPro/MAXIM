...
describe('bot speech styles', () => {
  ...
  it('renders robot, friendly and ironic templates with style presets', () => {
    ...
    expect((service as any).buildGreetingMessage(userLabel, '', 'ROBOT')).toBe(
      'Система: **Алексей**. Доступ подтвержден. Активный модератор: Майор Максимов.',
    );
    expect(
    ...
    expect((service as any).buildLinkExplanation(userLabel, true, '', 'ROBOT')).toBe(
      'Система: **Алексей**. Ссылка удалена. Основание: в этом чате ссылки не проходят, без ссылок.',
    );
    expect(
      ...
      'Система: **Алексей**. Предупреждение зарегистрировано. Для отправки сообщений по-прежнему требуется подписка на Новости MAX. Причина: для сообщений нужна подписка на обязательные каналы.',
    );
    ...
    expect(
      ...
      'Система: **Алексей**. Сообщение отклонено. Основание: слишком длинное сообщение: 187 символов при лимите 100.',
    );

    expect((service as any).buildDuplicateHitExplanation(userLabel, true, '', 'ROBOT')).toBe(
      'Система: **Алексей**. Повтор сообщения подтвержден. Сообщение удалено.',
    );
    ...
    ).toBe('Система: **Алексей**. Повтор сообщения подтвержден. Предупреждение зарегистрировано.');
    ...
    expect(
      ...
      'Система: **Алексей**. Предупреждение зарегистрировано. Основание: слишком длинное сообщение.',
    );
    ...
    expect((service as any).buildGreetingMessage(userLabel, '', 'IRONIC')).toBe(
      '**Алексей**, добро пожаловать 🙂 На смене Майор Максимов. Правила тут работают без выходных.',
    );
    expect(
    ...
    expect((service as any).buildLinkExplanation(userLabel, true, '', 'IRONIC')).toBe(
      '**Алексей**, ссылку убрал. Интернет большой, но именно сюда её тащить не нужно.',
    );

    expect((service as any).buildDuplicateHitExplanation(userLabel, true, '', 'IRONIC')).toBe(
      '**Алексей**, это сообщение уже появлялось. Повтор убрал. Коллекцию можно не собирать.',
    );
    ...
    ).toBe('**Алексей**, это сообщение уже появлялось. Да, это уже предупреждение.');
  });
  ...
  it('keeps system notices on the selected base style when one editable field is overridden', () => {
    ...
    expect((service as any).buildLinkMuteExplanation(userLabel, 'IRONIC')).toBe(
      '**Алексей**, со ссылками вышел повторный сезон, поэтому выдан мут.',
    );
  });
