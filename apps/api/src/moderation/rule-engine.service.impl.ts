...
const ADS_RECRUITMENT_PATTERNS: LabeledPattern[] = [
  ...
  },
  {
    label: 'роль-условия',
    pattern:
      /(?:^|[^\p{L}\p{N}_-])(?:повар[\p{L}\p{N}_-]*|охранник[\p{L}\p{N}_-]*|водител[\p{L}\p{N}_-]*|оператор[\p{L}\p{N}_-]*|модератор[\p{L}\p{N}_-]*|тестировщик[\p{L}\p{N}_-]*|упаковщик[\p{L}\p{N}_-]*|комплектовщик[\p{L}\p{N}_-]*|работник[\p{L}\p{N}_-]*|сотрудник[\p{L}\p{N}_-]*)(?:[\p{L}\p{N}\s.,:;()/%+-]{0,110})(?:ставк[\p{L}\p{N}_-]*|график[\p{L}\p{N}_-]*|оплат[\p{L}\p{N}_-]*|смен[\p{L}\p{N}_-]*|звоните|пишите|ваканси[\p{L}\p{N}_-]*)(?=$|[^\p{L}\p{N}_-])/iu,
  },
  {
...
const ADS_SERVICE_SPECIALTY_PATTERNS: LabeledPattern[] = [
  ...
  },
  {
    ...
    pattern:
      /(?:^|[^\p{L}\p{N}_-])(?:кадастров[\p{L}\p{N}_-]*|межеван[\p{L}\p{N}_-]*)(?:[\p{L}\p{N}\s.,:;()/%+-]{0,140})(?:центр|услуг[\p{L}\p{N}_-]*|участк[\p{L}\p{N}_-]*|тел\.?|телефон|звон[\p{L}\p{N}_-]*)(?=$|[^\p{L}\p{N}_-])/iu,
  },
  {
    label: 'подъем-домов',
    pattern:
