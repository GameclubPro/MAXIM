...
const ADS_HIGH_RISK_COMMERCIAL_PATTERNS: LabeledPattern[] = [
  ...
  },
  {
    ...
    pattern:
      /(?:^|[^\p{L}\p{N}_-])(?:отправьте\s+\d+\s+сообщени[\p{L}\p{N}_-]*|поиск\s+клиент[\p{L}\p{N}_-]*|заявк[\p{L}\p{N}_-]*\s+каждый\s+день|т[её]пл[\p{L}\p{N}_-]*\s+баз[\p{L}\p{N}_-]*)(?:[\p{L}\p{N}\s.,:;()/%+-]{0,90})(?:клиент[\p{L}\p{N}_-]*|заявк[\p{L}\p{N}_-]*|подарок|систем[\p{L}\p{N}_-]*|ссылк[\p{L}\p{N}_-]*)?(?=$|[^\p{L}\p{N}_-])/iu,
  },
  {
