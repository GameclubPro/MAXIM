...
describe('AdminService.listChannels', () => {
  ...
  it('merges the current channel into cached allowlist results on default load', async () => {
    const prisma = createPrismaMock();
    prisma.chatAdminAllowlist.findMany.mockResolvedValue([
      {
        chat: {
  ...
  it('returns cached chats immediately and skips blocking discovery when deferred discovery is enabled', async () => {
    ...
    const result = await service.listChats(user, {
      deferDiscovery: true,
    });
  ...
  it('returns refresh state without blocking the initial response when deferred discovery is enabled', async () => {
    const prisma = createPrismaMock();
    prisma.chatAdminAllowlist.findMany.mockResolvedValue([]);
    ...
    const result = await service.listChatsWithRefreshState(user, {
      deferDiscovery: true,
    });
