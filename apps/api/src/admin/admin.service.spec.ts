...
describe('AdminService.publishChannelEngagementMessage', () => {
  ...
  it('imports handoff attachments into channel comments and clears them after send', async () => {
    const prisma = createPrismaMock();
    prisma.chat.findUnique.mockResolvedValue({
      entityType: 'CHANNEL',
    });
    ...
    prisma.auditLog.create.mockResolvedValue({
      id: 'channel-comment-handoff-1',
      actorUserId: 'user-1',
      payload: {},
      createdAt: new Date('2026-03-20T10:13:00.000Z'),
    });
    ...
    const commentsToken = await publishCommentsDialogToken(service, maxClient);
    const handoffState = await handoffService.appendAttachments(
      'user-1',
      {
        chatId: 'channel-1',
        entityType: 'channel',
      ...
      [
        {
          kind: 'image',
          payload: {
            token: 'imported-image-1',
            url: 'https://cdn.max.ru/imported-image-1.png',
            width: 960,
            height: 720,
          },
          mimeType: 'image/png',
        ...
        },
        {
          kind: 'file',
          payload: {
            token: 'imported-file-1',
            url: 'https://cdn.max.ru/imported-file-1.pdf',
    ...
    const result = await service.createChannelDialogMessage(
      ...
      'comments',
      {
        token: commentsToken,
        text: '',
        handoffAttachmentIds: handoffState.attachments.map((attachment) => attachment.id),
      },
    );
    ...
    await expect(
      handoffService.getState('user-1', {
        chatId: 'channel-1',
        entityType: 'channel',
        token: commentsToken,
      }),
