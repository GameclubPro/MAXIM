...
export function PublisherEntitiesPage({
  ...
  function selectView(nextView: PublisherEntityView): void {
  ...
  function renderEntity(entity: PublisherEntity, renderedIndex: number) {
    ...
    const capabilities = getPublisherEntityCapabilities(entity);
    const entityKey = `${entity.entityType}:${entity.id}`;
  ...
  return (
    ...
    >
      ...
      <SegmentedControl
        value={view}
        options={[
          { value: 'chat', label: 'Чаты', count: summary.chat },
          { value: 'channel', label: 'Каналы', count: summary.channel },
        ]}
        onChange={selectView}
        className="publisher-entities-page__tabs"
        ariaLabel="Тип получателей"
      />
      ...
      {searchSettling ? (
        ...
        <div className="publisher-entities-page__state" role="status">
          ...
          <span>Откройте основной бот, проверьте доступ администратора и обновите список.</span>
          <button
            type="button"
            onClick={() =>
              void queryClient.resetQueries({ queryKey: entitiesQueryKey, exact: true })
      ...
      {!searchSettling && entities.length > 0 && entitiesQuery.hasNextPage ? (
        <button
          ...
          onClick={() =>
            void (entitiesQuery.isFetchNextPageError
