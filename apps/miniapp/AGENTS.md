...
- Publik entity module screens own chat comments, channel suggestions, and VK import. VK transport uses only `/publisher/entities/:entityType/:entityId/vk-parsing`. Major keeps channel comments, but its routes, deep links, settings state, API clients, search aliases, and CSS must not contain VK parsing controls or compatibility paths.
